...
NOTE: cell type deconvolution results from **SpatialDWLS**, **CARD** and **DestVI** are 1 spot fewer than all 293 spatial spots.
...
for (method_name in all_methods) {
  if (method_name %in% c('SpatialDWLS', 'CARD', 'DestVI')) {
...
if (save_file) {
  file_name = file.path(home.dir, 'FigS6_Melanoma_7celltype_marker_heatmap.pdf')
...
for (method_name in all_methods) {
  g_list[[method_name]] = CARD::CARD.visualize.pie(
    ...
    colors = melanoma_color, radius=0.55) + theme(legend.position="none", title=element_text(face=2, size=12)) + ggtitle(method_name)
}

# get the legend
g_list[[1]] = g_list[[1]] + theme(legend.position="right") + scale_fill_manual(values=melanoma_color, breaks=names(melanoma_color), labels=celltype_label) + guides(fill=guide_legend(title="Cell-type"))
...
if (save_file) {
  file_name = file.path(home.dir, 'FigS7_Melanoma_celltype_prop_heatmap_impute.pdf')
...
if (save_file) {
  file_name = file.path(home.dir, 'FigS8_Melanoma_4region_marker_heatmap_impute_allmethods.pdf')
