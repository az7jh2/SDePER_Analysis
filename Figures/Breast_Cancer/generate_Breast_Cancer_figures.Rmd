...
if (save_file) {
  file_name = file.path(home.dir, 'FigS9_Breast_Cancer_9celltype_marker_heatmap.pdf')
...
for (method_name in all_methods) {
  g_list[[method_name]] = CARD::CARD.visualize.pie(
    ...
    colors = bc_color, radius=0.55) + theme(legend.position="none", title=element_text(face=2, size=12)) + ggtitle(method_name) + scale_y_reverse()
}

# get the legend
g_list[[1]] = g_list[[1]] + theme(legend.position="right") + scale_fill_manual(values=bc_color, breaks=names(bc_color), labels=celltype_label) + guides(fill=guide_legend(title="Cell-type"))
...
for (this_diameter in c('160', '114', '80')) {
  for (method_name in c('SDePER', 'CARD')) {
    g_list[[length(g_list)+1]] = CARD::CARD.visualize.gene(
      ...
      NumCols = 4) +
      geom_point(aes(color=value), size=3, shape=15, position=position_dodge2(width=0.05)) +
      theme(legend.position="right", plot.subtitle=element_text(size=14)) + labs(subtitle = sprintf('%s μm', this_diameter)) + scale_y_reverse()
  }
}
