---
title: "Integrate STARmap-based simulated spots and reference scRNA-seq cells by Seurat"
author: "Ningshan Li"
date: "2024/09/11"
output:
  html_notebook:
...
home.dir = '/home/hill103/Documents/Spatial/Simulation'
save_file = FALSE
...
1.  **Inputs**:
    ...
    * [`sim_spatial_spot_nUMI.csv`](https://github.com/az7jh2/SDePER_Analysis/blob/main/Simulation/Generate_simulation_data/sim_spatial_spot_nUMI.csv): raw nUMI counts of simulated spatial transcriptomic data (spots × genes).
...
2.  **Outputs**:
    ...
    * [Seurat_integrated_spatial_spot_nUMI.csv](https://github.com/az7jh2/SDePER_Analysis/blob/main/Simulation/Run_SDePER_on_simulation_data/Scenario_1/ref_scRNA_seq/Seurat_integrated_spatial_spot_nUMI.csv): nUMI-like gene expression counts of STARmap-based simulated spots after Seurat integration.
    * [Seurat_integrated_ref_scRNA_nUMI.csv](https://github.com/az7jh2/SDePER_Analysis/blob/main/Simulation/Run_SDePER_on_simulation_data/Scenario_1/ref_scRNA_seq/Seurat_integrated_ref_scRNA_nUMI.csv): nUMI-like gene expression counts of scRNA-seq reference cells after Seurat integration.
...
### STARmap-based simulated spots nUMI

```{r}
file_name = file.path(home.dir, 'sim_spatial_spot_nUMI.csv')
starmap_exp = read.csv(file_name, row.names = 1, check.names = F, stringsAsFactors = F)

print(sprintf('load data from file %s', file_name))
print(sprintf('Total spots: %d; genes: %d', nrow(starmap_exp), ncol(starmap_exp)))
```
...
```{r}
obj_starmap = CreateSeuratObject(counts = t(as.matrix(starmap_exp)), min.cells = 0, min.features = 0)
print(sprintf('Total spots: %d; genes: %d', ncol(obj_starmap), nrow(obj_starmap)))
...
print(sprintf('finall keep Spatial spots: %d; genes: %d', ncol(obj_starmap), nrow(obj_starmap)))
...
To be consistent with SDePER which use gene nUMI counts as input, we applied an `expm1` transformation on Seurat integrated normalized gene expressions, followed by rounding to the nearest integer. 
...
norm_exp = round(expm1(obj_combined@assays$integrated@data))
...
print(sprintf('get expression of %d cells/spots and %d genes', ncol(norm_exp), nrow(norm_exp)))
```
...
```{r}
umap_embed = umap::umap(t(as.matrix(norm_exp) / 10000))
print(umap_embed)
```
...
# Save integrated values by Seurat

nUMI of Spatial spots after Seurat integration.

```{r}
file_name = file.path(home.dir, 'Seurat_integrated_spatial_spot_nUMI.csv')

to_save = t(as.matrix(norm_exp[, obj_combined$dataset=='Spatial']))
stopifnot(all(row.names(to_save) == row.names(starmap_exp)))

write.csv(to_save, file_name, row.names = T)
print(sprintf('Save expression of %d genes and %d spots to file %s', ncol(to_save), nrow(to_save), file_name))
```


nUMI of reference scRNA-seq cells after Seurat integration.

```{r}
file_name = file.path(home.dir, 'Seurat_integrated_ref_scRNA_nUMI.csv')

to_save = t(as.matrix(norm_exp[, obj_combined$dataset=='Reference']))
stopifnot(all(row.names(to_save) == row.names(ref_ct)))

write.csv(to_save, file_name, row.names = T)
print(sprintf('Save expression of %d genes and %d cells to file %s', ncol(to_save), nrow(to_save), file_name))
```
