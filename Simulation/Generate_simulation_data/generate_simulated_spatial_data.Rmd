...
* `X`, `Y`: physical locations of cells. We manually add a **4500 pixels offset** on `X` coordinate of cells from 1st experiment
...
The physical location of pseudo-spots are saved in the column `x` and `y` in dataframe `starmap_res$prop_grid`, we extract them and store into a dataframe named `local_df`
...
physical locations of all pseudo-spots are stored in `local_df`, and saved into file [`sim_spatial_spot_loc.csv`](https://github.com/az7jh2/CVAE-GLRM_Analysis/blob/main/Simulation/Generate_simulation_data/sim_spatial_spot_loc.csv)
...
Adjacency Matrix are stored in `A`, and saved into file [`sim_spatial_spot_adjacency_matrix.csv`](https://github.com/az7jh2/CVAE-GLRM_Analysis/blob/main/Simulation/Generate_simulation_data/sim_spatial_spot_adjacency_matrix.csv)
...
True cell type proportions of simulated spatial pseudo-spots are stored in `starmap_res$prop_grid`, and saved into file [`sim_spatial_spot_true_prop.csv`](https://github.com/az7jh2/CVAE-GLRM_Analysis/blob/main/Simulation/Generate_simulation_data/sim_spatial_spot_true_prop.csv)
...
nUMI count of all cells in STARmap dataset are stored in `gene_count` with **rows as cells and columns as genes**, and saved into file [`STARmap_cell_nUMI.csv`](https://github.com/az7jh2/CVAE-GLRM_Analysis/blob/main/Simulation/Analysis_simulation_data/Scenario_1/No_platform_effect/STARmap_cell_nUMI.csv)
...
Cell type annotation of all cells in STARmap dataset are stored in `meta_data$cluster_name`, and saved into file [`STARmap_cell_celltype.csv`](https://github.com/az7jh2/CVAE-GLRM_Analysis/blob/main/Simulation/Analysis_simulation_data/Scenario_1/No_platform_effect/STARmap_cell_celltype.csv)
